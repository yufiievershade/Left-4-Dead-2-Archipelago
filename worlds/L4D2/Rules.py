...
def set_rules(world: "L4D2World"):
   ...
   # 1. Access rules for campaign entrances
    ...
    campaign_names = [
        ...
        "The Parish", "The Sacrifice", "No Mercy", "Crash Course", "Death Toll",
    ...
    for campaign in campaign_names:
        ...
        add_rule(MultiWorld.get_entrance(entrance_name, player),
    ...
    if required_campaigns >= len(campaign_names):
        ...
        add_rule(MultiWorld.get_location("Beat Campaigns", player),
                 lambda state: all(state.has(f"Campaign: {c}", player) for c in campaign_names))
    else:
        # Otherwise, require the specified number of any campaigns
        add_rule(MultiWorld.get_location("Beat Campaigns", player),
                 lambda state: sum(1 for c in campaign_names if state.has(f"Campaign: {c}", player)) >= required_campaigns)
    ...
    def has_required_campaigns(state):
        owned_campaigns = sum(1 for c in campaign_names if state.has(f"Campaign: {c}", player))
