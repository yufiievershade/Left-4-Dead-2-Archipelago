...
def create_regions(world: MultiWorld, options: L4D2Options, player: int) -> None:
    ...
    for location_name, location_data in location_table.items():
        ...
        if region_name and region_name in regions:
            ...
            location = L4D2Location(
                player, location_name, location_data.code, menu_region
            )
    ...
    for campaign_name in [
        ...
        menu_to_campaign.access_rule = lambda state, c=campaign_name: state.has(
            f"Campaign: {c}", player
        )
        regions["Menu"].exits.append(menu_to_campaign)

        # Add an entrance for returning to the menu from the campaign
        campaign_to_menu = Entrance(
            player, f"{campaign_name} to Menu", regions[campaign_name]
        )
