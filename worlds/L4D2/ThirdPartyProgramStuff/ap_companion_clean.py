...
def find_all_l4d2_paths():
    ...
    if not found_paths:
        ...
        while True:
            user_path = input(
                "Please enter your L4D2 installation path (e.g., C:\\Program Files (x86)\\Steam\\steamapps\\common\\Left 4 Dead 2): "
            )
...
class L4D2CompanionGUI(tk.Tk):
    def __init__(self):
        ...
        self.disconnect_button = ttk.Button(
            button_frame, text="Disconnect", command=self.disconnect, state=tk.DISABLED
        )
        ...
        self.campaigns_text = tk.Text(
            campaigns_inner, height=20, width=70, state=tk.DISABLED, wrap=tk.WORD
        )
    ...
    def _apply_theme(self):
        if self.is_dark:
            ...
            self.style.configure(
                "TLabelFrame.Label", background=bg_dark, foreground=fg_light
            )
            self.style.configure("TButton", background="#44475a", foreground=fg_light)
            self.style.configure(
                "TEntry", fieldbackground="#44475a", foreground=fg_light
            )
            ...
            self.style.configure(
                "TLabelFrame.Label", background="white", foreground="black"
            )
    ...
    def toggle_theme(self):
        ...
        self.log_message(
            f"Theme switched to {'Dark' if self.is_dark else 'Light'} mode", "info"
        )
    ...
    def connect(self):
        ...
        self.connection_thread = threading.Thread(
            target=self._run_connection, args=(host, slot, password), daemon=True
        )
    ...
    def _on_connect_fail(self, error_message):
        ...
        messagebox.showerror(
            "Connection Failed", f"Could not connect to server:\n{error_message}"
        )
...
def update_status(player_name, connected=True):
    ...
    status = {
        ...
        "locked_weapon_upgrades": [
            w for w in ALL_WEAPON_UPGRADES if w not in unlocked_weapon_upgrades
        ],
        "unlocked_t1_weapons": unlocked_t1_weapons,
        "locked_t1_weapons": [
            w for w in ALL_T1_WEAPONS if w not in unlocked_t1_weapons
        ],
        "unlocked_t2_weapons": unlocked_t2_weapons,
        "locked_t2_weapons": [
            w for w in ALL_T2_WEAPONS if w not in unlocked_t2_weapons
        ],
        "unlocked_heavy_weapons": unlocked_heavy_weapons,
        "locked_heavy_weapons": [
            w for w in ALL_HEAVY_WEAPONS if w not in unlocked_heavy_weapons
        ],
        ...
        "locked_explosives": [
            e for e in ALL_EXPLOSIVES if e not in unlocked_explosives
        ],
    ...
    total_unlocked = (
        len(unlocked_campaigns)
        + len(unlocked_healing)
        + len(unlocked_weapon_upgrades)
        + len(unlocked_t1_weapons)
        + len(unlocked_t2_weapons)
        + len(unlocked_heavy_weapons)
        + len(unlocked_grenades)
        + len(unlocked_melee)
        + len(unlocked_explosives)
        + len(unlocked_misc)
    )
...
def write_trap_command(trap_type):
    ...
    for l4d2_path in L4D2_PATHS:
        ...
        possible_paths = [
            ...
            os.path.join(
                l4d2_path, "left4dead2", "addons", "sourcemod", "data", "archipelago"
            ),
            os.path.join(
                l4d2_path, "left4dead2"
            ),  # For ../../../trap_command.txt relative to scripting
        ]
...
def parse_printjson_message(msg):
    ...
    if "type" in msg:
        ...
        if "text" in msg:
            ...
            text = re.sub(
                r"694200+(\d+)",
                lambda m: ITEM_ID_TO_NAME.get(int(m.group(0)), m.group(0)),
                text,
            )
...
def extract_campaign_from_location_name(location_name):
    ...
    for camp in ALL_CAMPAIGNS:
        if (
            prefix.replace(" ", "")
            .lower()
            .startswith(camp.replace(" ", "").lower()[:5])
        ):
            return camp
    return None
...
async def check_goal_completion(websocket, player_name):
    ...
    if len(completed_campaigns) >= goal_campaigns:
        goal_completed = True
        log(
            f"GOAL COMPLETED! {len(completed_campaigns)}/{goal_campaigns} campaigns finished!",
            "success",
        )
...
async def handle_server_message(message, player_name, websocket):
    ...
    if cmd == "ReceivedItems":
        ...
        for item in items:
            ...
            if item_id and item_id in ITEM_ID_TO_NAME:
                ...
                if item_name.startswith("Trap: "):
                    ...
                    if trap_type > 0:
                        log(
                            f"Trap activated: {item_name.replace('Trap: ', '')}",
                            "error",
                        )
                        write_trap_command(trap_type)
                        continue
        ...
        if start_campaign_option is None:
            start_campaign_option = options.get(
                "StartWithCampaign", 1
            ...
            try:
                with open(GOAL_COMPLETED_FILE, "r") as f:
                    ...
                    if content == "1":
                        ...
                        try:
                            await send_packet(
                                websocket, [{"cmd": "StatusUpdate", "status": 30}]
                            )
                        except Exception as e:
        ...
        try:
            checked_locations = message.get("checked_locations", []) or message.get(
                "checked", []
            )
            seeded = 0
    ...
    while True:
        ...
        try:
            ...
            if msg:
                try:
                    ...
                    if isinstance(message, list):
                        for m in message:
                            response = await handle_server_message(
                                m, player_name, websocket
                            )
                            if response:
                                await send_packet(websocket, response)
                    else:
                        response = await handle_server_message(
                            message, player_name, websocket
                        )
                        if response:
                            await send_packet(websocket, response)
            ...
            if iteration % 1000 == 1:
                if DEBUG:
                    print(
                        f"Polling for location_check.txt in {len(L4D2_PATHS)} paths: {L4D2_PATHS}"
                    )
            if iteration % 100 == 0:
                if DEBUG:
                    print(
                        f"Heartbeat: Connected=True, Completed Campaigns={len(completed_campaigns)}, Goal Progress={len(completed_campaigns)}/{goal_campaigns}, Goal Completed={goal_completed}"
                    )
            ...
            for l4d2_path in L4D2_PATHS:
                ...
                if os.path.exists(location_file):
                    try:
                        ...
                        location_name = get_location_name_from_id(location_id)
                        if location_name and "Finale" in location_name:
                            campaign = extract_campaign_from_location_name(
                                location_name
                            )
                            if campaign and campaign not in completed_campaigns:
                                completed_campaigns.add(campaign)
                                log(
                                    f"Finale completed: {campaign} ({len(completed_campaigns)}/{goal_campaigns})",
                                    "success",
                                )
                            await check_goal_completion(websocket, player_name)
